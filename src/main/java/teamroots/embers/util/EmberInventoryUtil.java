...
public class EmberInventoryUtil {
	public static double getEmberCapacityTotal(EntityPlayer player){
		double amount = 0;
		for (int i = 0; i < 36; i ++){
			if (player.inventory.getStackInSlot(i) != EMPTY_ITEM_STACK){
		...
		if (player.getHeldItem(EnumHand.OFF_HAND) != EMPTY_ITEM_STACK){
		...
		if (player.getHeldItem(EnumHand.MAIN_HAND) != EMPTY_ITEM_STACK){
